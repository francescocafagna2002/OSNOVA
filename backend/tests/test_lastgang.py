...
def test_load_building_series_matches_synth(settings, synth_dir, truth):
    ...
    assert isinstance(gp_of[mid], int) and load_building_series(store, str(gp_of[mid])).height == out.height


def test_load_building_series_legacy_power_kw_layout(settings, synth_dir, truth):
    store = Store(settings)
    gp_of = write_synth_by_file(synth_dir, store, truth, years=(2024,), legacy_power_kw=True)
    mid = pick(truth, pv=True)
    assert pl.read_parquet(sorted(by_file_dir(store).glob("*.parquet"))[0]).schema["gp_nr"] == pl.String
    out = load_building_series(store, gp_of[mid])
    ref = load_synth_meter(synth_dir, mid, 2024).sort("ts")
    assert out.schema == BUILDING_SERIES and out.height == ref.height
    assert (out["export_kw"] - ref["export_kw"]).abs().max() < 1e-3
...
def test_load_building_series_marks_missing_and_unknown(settings, synth_dir, truth):
    ...
    df.with_columns(
        [
            pl.when(pl.col("gp_nr") == gp).then(None).otherwise(pl.col(c)).alias(c)
    ...
    empty = load_building_series(store, 0)
